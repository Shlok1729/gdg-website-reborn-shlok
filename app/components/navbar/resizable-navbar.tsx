  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "min-w-[calc(100%-(--spacing(4)))] md:min-w-[800px]",
        visible && "bg-card",
        className
      )}
    >
      ...
      {items.map((item, idx) => {
        if (item.type === "link") {
          return (
            <DefaultLink
              key={item.title}
              active={idx === hovered || pathname === item.href}
...
export const DropDownLink = ({
  children,
  ...rest
}: { children: React.ReactNode } & React.AnchorHTMLAttributes<
  HTMLAnchorElement
>) => {
  return (
    <a
...
export const NavbarLogo = () => {
  return (
    <a
      href="/"
      className="relative z-20 flex items-center gap-2 text-sm font-medium text-foreground"
    >
      <Image
        src="/knight.png"
        alt="logo"
        ...
        className="w-8 h-8"
      />
      <span className="font-Exo font-medium text-neutral-700 dark:text-neutral-300 hidden md:inline">
        DEVELOPERS CLUB
      </span>
    </a>
  );
